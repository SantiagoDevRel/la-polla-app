...
const config = {
  ...
  entryPriceCop: 10000, houseCutPct: 0, closesAt: "2026-09-14T17:00:00.000Z", closeMode: "manual",
...
const previewRequest = (query = "price=10000&cut=0&tickets=1000&kind=pozo&mode=fijo&fixed=1000000") =>
...
describe("fixed-prize creation and preview API", () => {
  ...
  it.each([
    ...
    { houseCutPct: 30 }, { prizeKind: "objeto", prizeObject: "Camiseta" }, { publishesAt: undefined },
  ...
  it("uses SQL's fixed preview numbers even when the house balance is negative", async () => {
    const result = { fixed_prize: 1000000, ten_prize: 1000000, all_prize: 1000000, ten_balance: -900000, all_balance: 9000000 };
    dbFetch.mockResolvedValueOnce(response(result));
    const res = await preview(previewRequest());
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual(result);
    expect(written()).toEqual({ p_price: 10000, p_fixed: 1000000, p_tickets: 1000 });
    expect(new URL(String(dbFetch.mock.calls[0][0])).pathname).toBe("/rest/v1/rpc/casa_fixed_prize_preview_v2");
  });

  it.each([
    "price=10000&cut=30&tickets=1000&kind=pozo&mode=fijo&fixed=1000000",
