...
// Event-type icon resolution. Moment types (rank_up/rank_down/
// perfect_pick/last_place) use the matching Pixa-generated video — the
// top-most unread row animates, everything else shows the poster frame
// so we never run 5+ loops at once. Structural events (polla_finished,
// polla_started) stay on lucide glyphs.
interface TypeVisual {
  /** Poster webp shown as a still image on non-animating rows. */
  posterSrc?: string;
  /** Looping video sources used on the top-most unread row only. */
  videoWebm?: string;
  videoMp4?: string;
  /** Fallback lucide icon for structural events. */
  Icon?: typeof Trophy;
  tint: string;
  ring: string;
  iconColor: string;
...
const TYPE_VISUALS: Record<NotificationType, TypeVisual> = {
  rank_up: {
    posterSrc: "/videos/la-polla-rankup-moment-poster.webp",
    videoWebm: "/videos/la-polla-rankup-moment.webm",
    videoMp4: "/videos/la-polla-rankup-moment-lite.mp4",
    tint: "bg-gold/15",
    ring: "border-gold/30",
    iconColor: "text-gold",
  },
  rank_down: {
    posterSrc: "/videos/la-polla-rivales-poster.webp",
    videoWebm: "/videos/la-polla-rivales.webm",
    videoMp4: "/videos/la-polla-rivales-lite.mp4",
    tint: "bg-red-alert/15",
    ring: "border-red-alert/30",
    iconColor: "text-red-alert",
  },
  perfect_pick: {
    posterSrc: "/videos/la-polla-celebration-poster.webp",
    videoWebm: "/videos/la-polla-celebration.webm",
    videoMp4: "/videos/la-polla-celebration-lite.mp4",
    tint: "bg-turf/15",
    ring: "border-turf/30",
    iconColor: "text-turf",
  },
  last_place: {
    posterSrc: "/videos/la-polla-triste-poster.webp",
    videoWebm: "/videos/la-polla-triste.webm",
    videoMp4: "/videos/la-polla-triste-lite.mp4",
    tint: "bg-bg-elevated",
...
export function AvisosList({ initialItems, initialUnread }: AvisosListProps) {
  ...
  return (
    <>
      ...
      {/* Feed. The first unread item animates its event video; every
          other row (read + older unread) renders the static poster so
          the page stays at one concurrent video loop. */}
      {items.length === 0 ? (
        <EmptyState />
      ) : (
        <ul className="flex flex-col gap-2">
          {(() => {
            let animatedGiven = false;
            return items.map((n) => {
              const isTopUnread = !animatedGiven && !n.read_at;
              if (isTopUnread) animatedGiven = true;
              return (
                <Aviso
                  key={n.id}
                  item={n}
  ...
  const content = (
    ...
    >
      {/* Icon well. Moment events render the Pixa poster frame, rank_down
          still promotes the actor's personal pollito when we know who
          overtook us, structural events fall back to a lucide glyph. */}
      {item.type === "rank_down" && item.actor?.avatar_url ? (
        <div className="relative w-10 h-10 rounded-full overflow-hidden flex-shrink-0 bg-bg-elevated border border-red-alert/30">
          <Image
        ...
        <div
          className={
            "relative w-10 h-10 rounded-full overflow-hidden flex-shrink-0 border " +
            visual.tint +
            " " +
            visual.ring
          }
        >
          {animateIcon && visual.videoWebm && visual.videoMp4 ? (
            <video
              autoPlay
              muted
              loop
