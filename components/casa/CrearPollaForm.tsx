...
import { DEFAULT_PICKER_TOURNAMENT, MatchPicker, type MatchOption, type PickedMatch } from "@/components/casa/MatchPicker";
...
type SelectedMatch = PickedMatch;
...
export function CrearPollaForm() {
  ...
  function toggleMatch(match: PickedMatch) {
    setSeleccion((prev) => {
  ...
  const refrescarElegidos = useCallback((loaded: MatchOption[], tournament: string) => {
  ...
  async function crear(publicar: boolean) {
    ...
    try {
      ...
      const body =
        kind === "partidos"
          ? { ...base, kind, tournament: seleccion[0]?.tournament ?? DEFAULT_PICKER_TOURNAMENT, scoringMode, matchIds: seleccion.map((m) => m.id) }
          : kind === "manual"
            ? {
                ...base,
                kind,
    ...
    <div className="space-y-5">
      ...
      {kind === "partidos" && (
        <MatchPicker
          ...
          maxSelected={30}
          onMatchesLoaded={refrescarElegidos}
