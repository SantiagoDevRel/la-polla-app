...
export default async function CasaPage() {
  ...
  // Siempre queda al menos una sección abierta: Mis pollas si la persona tiene
  // pollas en juego; disponibles si hay alguna o si no tiene nada en juego.
  ...
  return (
    <div className="pb-28">
      ...
      <div className="space-y-4 px-4 pt-6">
        <MyPollas initialPollas={enJuegoMias} activeOnly defaultOpen={misPollasOpen} pendingByPolla={Object.fromEntries(pendientes.map(p => [p.polla.id, p.faltan]))} />
        ...
        <PollaSection id="pollas-cerradas" kind="closed" title="Pollas cerradas" description="Consulta los resultados de pollas anteriores." count={cerradas.length}>
            {cerradas.length > 0 ? (
              <ul className="grid auto-rows-fr gap-3">
                {cerradas.map((polla) => (
                  <PollaRow key={polla.id} polla={polla} pot={pots[polla.id]} tournaments={tournaments[polla.id] ?? []} participated={joinedIds.has(polla.id)} closed />
                ))}
              </ul>
            ) : (
