  ...
  return (
    <div className="min-h-screen" style={{ background: "#080c10" }}>
      ...
      <main className="max-w-lg mx-auto p-4 space-y-4">
        ...
        {/* Deep links — test exploratorio. NO hay docs oficiales de
            URL schemes para apps bancarias colombianas; éstos son
            inferidos de issues de GitHub + reverse engineering.
            Si funciona desde tu iPhone → camino válido. Si no → discard. */}
        <DeepLinkTester
          method={method}
          account={account}
...
function DeepLinkTester({
  ...
  // Cada candidato es un URL scheme inferido o universal-link
  // candidato. Ninguno está documentado oficialmente — son intentos.
  // Si abre la app en iPhone → win. Si no → fallback al screenshot.
  const acctDigits = account.replace(/\D/g, "");
  const amt = amount.replace(/\D/g, "") || "0";
  const candidates: Array<{ label: string; url: string; note: string }> = [];

  if (method === "nequi") {
    candidates.push(
      {
        label: "Nequi · scheme directo",
        url: `nequi://send-money?phone=${acctDigits}&amount=${amt}`,
        note: "URL scheme custom — solo funciona si Nequi registró el handler.",
      },
      {
        label: "Nequi · web universal",
        url: `https://nequi.com.co/`,
        note: "Universal Link al sitio. iOS abre la app si tiene aaSA configurado.",
      },
    );
  } else if (method === "bancolombia") {
    candidates.push(
      {
        label: "Bancolombia · scheme directo",
        url: `bancolombia://transfer?account=${acctDigits}&amount=${amt}&name=${encodeURIComponent(recipientName)}`,
        note: "URL scheme inferido. Probablemente NO funciona en iOS sin app instalada y registrada.",
      },
      {
        label: "Bancolombia · comobancolombia",
        url: `comobancolombia://`,
        note: "Variante del scheme reportada en algunos foros.",
      },
      {
        label: "Bancolombia · web universal",
        url: `https://www.bancolombia.com.co/`,
        note: "Si Bancolombia configuró Universal Links en su app iOS, esto la abriría.",
      },
    );
  } else {
    candidates.push({
      label: "Sin candidates conocidos para 'Otro'",
      url: "",
      note: "Probá con Nequi o Bancolombia.",
    });
  }

  return (
    <section className="rounded-2xl p-4 lp-card space-y-3 border border-amber/20">
      <div>
        <h3 className="text-sm font-bold text-text-primary">
          Deep links · test desde iPhone
        </h3>
        <p className="text-[11px] text-text-muted mt-0.5">
          Tocá cada botón. Si abre la app del banco con los datos pre-llenados → ese URL scheme funciona y lo usamos. Si te lleva al browser, App Store, o no hace nada → ese candidato no sirve.
        </p>
      </div>

      {candidates.map((c, i) => (
        <div key={i} className="rounded-lg p-3 bg-bg-elevated border border-border-subtle space-y-2">
          <div className="flex items-center justify-between gap-2">
            <p className="text-[12px] font-semibold text-text-primary truncate">{c.label}</p>
            {c.url ? (
              <a
                href={c.url}
                className="text-[11px] font-semibold px-3 py-1.5 rounded-lg bg-amber/15 border border-amber/30 text-amber hover:bg-amber/20 transition-colors flex-shrink-0 whitespace-nowrap"
              >
                Probar
              </a>
            ) : null}
          </div>
          <p className="text-[10px] text-text-muted">{c.note}</p>
          {c.url ? (
            <code
              className="text-[10px] text-text-secondary block bg-bg-base border border-border-subtle rounded px-2 py-1 break-all"
      ...
      <p className="text-[10px] text-text-muted">
        Tip: completá el form arriba (cuenta + monto) antes de probar — los deep links usan esos valores.
      </p>
    </section>
  );
}
