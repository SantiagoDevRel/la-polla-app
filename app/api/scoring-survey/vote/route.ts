// app/api/scoring-survey/vote/route.ts — Registra el voto del usuario en la
// encuesta de sistema de puntos.
//
// Valida que el usuario sea participante PAGADO de la polla y que la
// encuesta siga abierta (scoring_survey_open=true) antes de aceptar el voto.
...
export async function POST(req: NextRequest) {
  ...
  const { data: polla } = await admin
    .from("pollas")
    .select("id, scoring_survey_open")
    .eq("id", parsed.pollaId)
    .maybeSingle();
  if (!polla || !polla.scoring_survey_open) {
  ...
  const { error: upErr } = await admin.from("scoring_survey_votes").upsert(
    {
  ...
  if (upErr) {
    console.error("[scoring-survey] vote upsert error:", upErr);
    return NextResponse.json(
