...
BEGIN
  fixed_pool:=pg_temp.publication_create(pg_temp.publication_config());
  SELECT * INTO pot FROM public.casa_pot_summaries_v2(ARRAY[fixed_pool]);
  ASSERT pot.paid_entries=0 AND pot.gross_cop=0 AND pot.prize_cop=1000000 AND pot.house_cop=-1000000;
  ...
  SELECT * INTO pot FROM public.casa_pot_summaries_v2(ARRAY[fixed_pool]);
  ASSERT pot.paid_entries=100 AND pot.gross_cop=1000000 AND pot.prize_cop=1000000 AND pot.house_cop=0;
  INSERT INTO public.casa_entries(polla_id,user_id,status,amount_cop)
    SELECT fixed_pool,id,'pagada',10000 FROM pg_temp.publication_people WHERE n BETWEEN 101 AND 1000;
  SELECT * INTO pot FROM public.casa_pot_summaries_v2(ARRAY[fixed_pool]);
  ASSERT pot.paid_entries=1000 AND pot.gross_cop=10000000 AND pot.prize_cop=1000000 AND pot.house_cop=9000000;
  ASSERT (SELECT prize_cop FROM public.casa_polla_pot(fixed_pool))=1000000;
  RAISE NOTICE 'PASS fixed prize exactly 1,000,000 at 0/100/1000 paid participants; balance is gross minus fixed';

  r:=public.casa_fixed_prize_preview_v2(10000,1000000,1000);
  ASSERT (r->>'ten_prize')::bigint=1000000 AND (r->>'all_prize')::bigint=1000000;
  ASSERT (r->>'ten_balance')::bigint=-900000 AND (r->>'all_balance')::bigint=9000000;
  ASSERT (r->>'entry_prize')::bigint=0 AND (r->>'entry_house')::bigint=10000;
  ...
  RAISE NOTICE 'PASS fixed preview and invalid amounts';
  ...
  -- Migration 107: a hidden pool commits no prize; a scheduled one keeps its
  -- commitment (it publishes itself at opens_at), and cannot be closed early.
  ...
  ASSERT (SELECT house_cop=-1000000 FROM public.casa_pot_summaries_v2(ARRAY[future_pool]));
  ASSERT public.casa_house_total_v2(ARRAY[hidden_pool,future_pool])=-1000000;
  ...
  RAISE NOTICE 'PASS hidden fixed pool has no commitment; closing an unpublished scheduled pool is rejected';
  ...
  PERFORM pg_temp.publication_must_fail(format('UPDATE public.casa_pollas SET house_cut_pct=30 WHERE id=%L',fixed_pool),'casa_fixed_prize_valid');
